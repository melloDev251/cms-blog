...
export class ApiService {
  ...
  constructor(private http: HttpClient, private router: Router) {
    this.http
      .get<{ status: boolean; user: User }>(`${this.URL}/auth/authstatus`)
      .toPromise()
      .then((res) => {
  ...
  removeCategory(id: number) {
    return this.http.delete<{ success: boolean; category: Category }>(
      `${this.URL}/category/${id}`,
      {
        withCredentials: true,
      }
  ...
  removePost(id: number) {
    return this.http.delete<{ success: boolean; post: Post }>(
      `${this.URL}/posts/${id}`,
      { withCredentials: true }
    );
  }
